...
Image *CreateNewSizeImage(Image * image, double factor, char *comment)
{
	Image *outimage;
	int size,j;
	...
	outimage->Type  = image->Type;
	if(outimage->Type==GRAY)   size  = (int)(image->Width * factor) * (int)(image->Height * factor);
	else if(outimage->Type==COLOR) size  = (int)(image->Width * factor) * (int)(image->Height * factor) * 3;

	outimage->Width = (int)image->Width * factor;
	outimage->Height = (int)image->Height * factor;
	outimage->num_comment_lines=image->num_comment_lines;
...
Image *CreateNewSizeImage(Image* image, int width, int height, char *comment)
{
	Image *outimage;
	int size,j;
